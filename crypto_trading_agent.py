# Система обучения агента для торговли криптовалютой в реальном времени
# Включает адаптивное обучение на ошибках, ансамбль моделей и управление рисками
...
def train_crypto_trading_agent(coin_name: str = "BTC"):
    """Обучает торгового агента для конкретной монеты"""
    print(f"Начинаем обучение агента для {coin_name}")
    
    # Получаем данные
    if coin_name not in coins_data:
        print(f"Монета {coin_name} не найдена в данных")
        return None
    
    data = coins_data[coin_name].get_dataset()
    print(f"Данные {coin_name}: {data.shape}")
...
if __name__ == "__main__":
    ...
    if btc_agent:
        ...
        # Показываем финальную производительность
        final_performance = btc_agent.get_performance_summary()
